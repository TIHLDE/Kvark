...
class Forum extends Component {
  ...
  handleSubmit = (event) => {
    ...
    const body =
      'Bedrift: ' +
      this.state.data.info.bedrift +
      '%0D%0A%0D%0A' +
      'Kontaktperson:%0D%0A' +
      'Navn: ' +
      this.state.data.info.kontaktperson +
      '%0D%0A' +
      'Epost: ' +
      this.state.data.info.epost +
      '%0D%0A%0D%0A' +
      'Valgt semester:%0D%0A' +
      this.arrayToList(this.state.data.time) +
      '%0D%0A' +
      'Valg arrangement:%0D%0A' +
      this.arrayToList(this.state.data.type) +
      '%0D%0A' +
      'Kommentar:%0D%0A' +
