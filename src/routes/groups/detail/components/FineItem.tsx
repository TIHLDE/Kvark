...
const FineItem = ({ fine, groupSlug, isAdmin, hideUserInfo, fineUser: _fineUser }: FineItemProps) => {
  ...
  const toggleApproved = () => {
    ...
    updateFine.mutate(
      { groupSlug, fineId: fine.id, data: { approved: !fine.approved } as never },
      {
        onSuccess: () => {
          toast.success(`Boten er na markert som ${fine.approved ? 'ikke godkjent' : 'godkjent'}`);
        },
        onError: (e) => {
  ...
  const togglePayed = () => {
    updateFine.mutate(
      { groupSlug, fineId: fine.id, data: { payed: !fine.payed } as never },
      {
        onSuccess: () => {
          toast.success(`Boten er na markert som ${fine.payed ? 'ikke betalt' : 'betalt'}`);
        },
        onError: (e) => {
          toast.error(e.message);
        },
  ...
  return (
    <Expandable
      ...
      <div className='space-y-4'>
        ...
        {/* TODO: Re-add user check for defense — previously checked (fineUser || fine.user)?.user_id === user?.user_id */}
        <FineDefense defense={fine.defense} fineId={fine.id} groupSlug={groupSlug} />
